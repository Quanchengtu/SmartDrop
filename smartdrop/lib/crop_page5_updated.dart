...
class _CropPageState extends State<CropPage> {
  ...
  Future<void> _submitData() async {
    ...
    try {
      ...
      print(
        "✅ API URL: https://gemini-api-101700959874.asia-east1.run.app/weather?lat=$lat&lon=$lon",
      );

      final weatherRes = await http.get(
        Uri.parse(
          'https://gemini-api-101700959874.asia-east1.run.app/weather'
          '?lat=${selectedLocation!.latitude}&lon=${selectedLocation!.longitude}',
        ),
      );

      final weatherData = jsonDecode(utf8.decode(weatherRes.bodyBytes));
...
當地天氣：
- 今日氣溫：${formatWeatherValue(weatherData['氣溫'])}
- 相對濕度：${formatWeatherValue(weatherData['濕度'])}
- 降雨機率：${formatWeatherValue(weatherData['降雨機率'])}
- 風速：${formatWeatherValue(weatherData['風速'])}
請針對上述資訊，建議：
1. 今日建議用水量
2. 適合灌溉的時間段，並簡要說明原因。
''';

      print("🌤️ weatherData: $weatherData");
      print("📩 Gemini prompt:\n$prompt");
      ...
      final decoded = utf8.decode(geminiRes.bodyBytes); // 這行強制用 UTF-8 解碼
      final geminiReply = jsonDecode(decoded);
      final suggestion = geminiReply['reply'] ?? '💧 預設建議：無法取得 Gemini 回應，請稍後再試';
...
String formatWeatherValue(dynamic value) {
  if (value == null || value.toString().toLowerCase().contains('無法查詢')) {
    return '未知';
  }
